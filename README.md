# Day Surgery Form Automator
# (日间手术随访表生成系统)

[![Python](https://img.shields.io/badge/Python-3.7+-blue.svg)](https://www.python.org/)
[![Version](https://img.shields.io/badge/Version-8.3-brightgreen.svg)]()
[![License](https://img.shields.io/badge/License-MIT-green.svg)](https://opensource.org/licenses/MIT)

这是一款功能强大且用户友好的医疗文档自动化工具，专为简化“日间手术随访表”的生成流程而设计。**V8.x 版本对项目代码进行了全面的模块化重构，将原先单一的脚本拆分为独立的逻辑 (`core`)、界面 (`gui`)、配置 (`config.py`) 和工具函数 (`utils.py`) 模块，大幅提升了代码的可读性与可维护性。**

![软件截图](./docs/images/screenshots/screenshot.png)
*<p align="center">软件截图</p>*

### 更新日志：[CHANGELOG](./docs/changelog.md)

---

## ✨ 核心功能

* ### 极致轻量、零依赖的内核
  * **SQLite 引擎驱动**: 全面采用 Python 内置的 `SQLite` 内存数据库进行数据处理，无需任何额外的第三方数据处理库，从根本上解决了程序打包体积过大的问题。
  * **多文件合并处理**: 支持一次性添加多个`手术查询文件`，程序会自动读取并整合所有文件中的床号信息，轻松应对数据分散在不同表格中的情况。
  * **强大的数据清洗与匹配**: 能够自动处理两个Excel文件中因格式差异（如住院号`0123` vs `123`）或隐藏字符（如不可见空格）导致的数据不一致问题，极大地提高了床号匹配的成功率。
  * **智能标题行检测**: 程序能自动识别Excel文件中真正的标题行在第几行，无视文件顶部的任何大标题或空行，确保数据读取的准确性。
  * **自动化日期处理**: 无需再手动选择“主导月份”。程序会为每位患者自动提取其真实的“出院日期”，并以此为准生成文件名和文档内容，实现完全自动化的精确处理。
  * **精准的失败提醒**: 独有的智能提醒机制，只针对符合“日间手术”条件的患者进行床号匹配失败的汇总提醒，过滤无关信息，让问题定位更快速、更精准。

* ### 现代化的用户体验 (UI/UX)
  * **专业启动画面**: 支持自定义启动画面，并在主程序就绪后无缝切换。
  * **自适应DPI**: 自动识别高分屏(Retina)，并等比例缩放窗口布局，确保在任何显示器上都获得清晰的视觉效果。
  * **多线程处理**: 将耗时的文件生成任务放在独立的后台线程中执行，确保GUI在处理过程中始终保持响应，不卡顿。
  * **可中断任务**: 在文档生成期间，可随时点击“停止生成”按钮以安全地中断任务。
  * **实时进度反馈**: 使用进度条和带颜色区分（提示/警告/错误）的滚动日志框，实时显示处理进度。
  * **日志管理**: 支持右键菜单清空或导出当前日志为`.txt`文件。

* ### 灵活的配置与高可维护性
  * **中央配置系统**: 所有关键参数（如日间手术天数定义、Excel列名映射等）均提取到 `src/config.py` 文件中，方便未来快速调整，无需修改核心代码。
  * **多线程处理**: 将耗时的文件生成任务放在独立的线程中执行，确保了GUI在处理过程中始终保持响应，提升了用户体验。
  * **内置模板系统**: 程序内置了标准的Excel和Word模板，可通过UI随时查看。当用户未指定外部Word模板时，将自动使用内置模板。
  * **安全模板预览**: 查看模板时，程序会创建一份临时的只读副本供用户查看，确保原始模板文件不会被意外修改。
  * **模块化架构**: 项目代码被全面重构，拆分为独立的界面(`gui`)、核心逻辑(`core`)、配置(`config.py`)等模块，结构清晰，极大地提高了代码的可读性和可维护性。

* ### 清晰的项目结构
    * **模块化设计**: 代码被拆分为配置、工具、核心逻辑和界面等模块，结构清晰，易于维护。
    * **中央化配置**: 所有关键参数集中在 `src/config.py` 文件中，方便快速调整。
    * **多线程处理**: 核心任务在后台线程运行，避免界面卡顿，提升用户体验。

## 📂 项目结构

```
.
├── LICENSE                   # 许可协议
├── README.md                 # 本说明文件
├── docs/                     # 存放文档和相关资源
│   ├── BUILD_GUIDE.md        # 程序打包指南
│   ├── changelog.md          # 更新日志
│   └── images/               # 文档用图片 (如截图)
│       ├── icons/            # 可选图标
│       └── screenshots/      # 软件截图
├── source/                   # 源代码目录
├── main.py               # 🚀 主程序入口
├── requirements.txt      # 📦 依赖列表
├── assets/               # 🎨 存放资源文件
│   ├── app.ico           # 程序图标
│   └── splash.png        # 启动画面
├── templates/            # 📂 内置模板文件夹
│   ├── 出院患者列表模板.xls
│   ├── 手术查询模板.xlsx
│   └── 日间手术随访登记表模板.docx
└── src/                  # 模块目录
    ├── __init__.py       # 将src声明为可导入的包
    ├── config.py         # 全局配置
    ├── temp_manager.py   # 临时文件管理器
    ├── template_handler.py   # 内置模板管理器
    ├── utils.py          # 工具函数
    ├── core/             # 核心逻辑模块
    │   ├── __init__.py
    │   ├── logic.py      # 核心业务编排
    │   └── modules/      # 核心功能子模块
    │       ├── __init__.py
    │       ├── db_manager.py     # 数据库管理
    │       ├── doc_writer.py     # 文档生成
    │       └── excel_reader.py   # Excel读取
    └── gui/              # GUI界面模块
        ├── __init__.py
        ├── app_controller.py   # GUI应用控制器 (原main_app.py)
        ├── handlers.py         # GUI事件处理器
        ├── tooltip.py          # 悬停提示工具
        ├── ui_builder.py       # GUI界面布局
        └── ui_components.py    # GUI可复用组件
```

## 🚀 安装与运行

详细的安装、运行及打包说明，请参考： **[自动化程序编译与打包指南](./docs/BUILD_GUIDE.md)**

## 🔧 配置说明

本程序的所有关键配置项均位于 `src/config.py` 文件中，您可以根据需要进行修改：

* `app_title`: 窗口标题。
* `day_surgery_max_days`: 定义日间手术的最大住院天数。
* `follow_up_days`: 定义随访发生于出院后的天数。
* `welcome_message`, `welcome_tips`, `welcome_warning`: 自定义程序启动时在日志区显示的欢迎语。
* `unknown_bed_placeholder`: 定义在Word文档内容中，当床号未知时显示的占位符。
* `unknown_bed_filename_suffix`: 定义当床号未知时，在文件名末尾添加的后缀。
* `column_mapping`: **核心配置**。定义了程序内部字段名与您Excel文件中列名的对应关系。如果您的Excel列名有变，只需修改此处的中文部分即可。
* `required_patient_cols` / `required_surgery_cols`: 定义了哪些字段是生成文档所必需的。
* `template_placeholders`: 定义了Word模板中的占位符与程序内部字段的对应关系。

## 👨‍💻 贡献者

* **原始作者与核心需求**: 顾江江
* **重构与AI辅助开发**: Google AI

## 📜 许可协议

本项目采用 [MIT License](https://opensource.org/licenses/MIT) 开源协议。